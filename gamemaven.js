...
// Put up a stern warning if the user is running IE8 (Game Maven doesn't do IE8)
...
// Save out the code the student was working on for each lesson.  This
// allows the student to get back to their code even after advancing or
// jumping to a new section, at least for the browser session, and
// avoids a common frustration where the code the student was working
// on just disappears.
var savedCode = {};
...
function updatePreview() {
	...
	// Every time we execute new code, save out the code.  This is later used
	// when we switch lessons (in initLesson()) to restore the code the student
	// was working on
	savedCode[currentLesson] = code;

	// Verify the code is okay and show errors, if any
...
var currentLessonStorageName = "codeGamerCurrentLesson";
...
function initLesson() {
	...
	// Use savedCode if available, otherwise use lesson.code
	// Saved code is just the code the student was working on in earlier lessons.
	// It's frustrating for students to have their code disappear, this makes
	// it persist, at least for the session (not across browser sessions though).
	var code = savedCode[currentLesson] || lesson.code;
	if (code != null) {
